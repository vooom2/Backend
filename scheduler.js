...
const { CREATE_PAYMENT_FUNCTION, CREDIT_VEHICLE_OWNERS } = require('./controllers/paymentController');
const { CREATE_INSPECTION_FUNCTION } = require('./controllers/vehicleController');
...
schedule.scheduleJob(dailyRule, async () => {
  ...
  riders.forEach((rider) => {
    const riderPayment = payments.filter(payment => `${payment.rider}` === `${rider._id}`);
    ...
    if (paymentReliabilityScore < 60) {
      ...
      if (rider.rider_rating.payment_reliability !== "Poor") {
        SEND_NOTIFICATION_FUNCTION(
          rider._id,
          `Payment Rating Changed to Poor`
        );
      }
      ...
      if (rider.rider_rating.payment_reliability !== "Good") {
        SEND_NOTIFICATION_FUNCTION(
          rider._id,
          `Payment Rating Changed to Good`
        );
      }
    ...
    const defaultedPayments = riderPayment.filter((payment) => payment.payment_status === 'overdue').length;
...
schedule.scheduleJob(weeklyRule, async () => {
  console.log('Running weekly job (every hour for testing)');
  // Get active vehicles
  const vehicles = await Vehicle.find({ rider: { $ne: null }, verified_vehicle: true, active_vehicle: true });

  // Create payments for each vehicle
  vehicles.forEach((vehicle) => {
    const rider = vehicle.rider;
    const paymentAmount = 10000; // Replace with actual amount
    const description = 'Weekly payment'; // Replace with actual description
...
schedule.scheduleJob(biWeeklyRule, async () => {
  ...
  const lapDescription = `${currentMonth}/${weekOfMonth <= 2 ? '1st lap' : '2nd lap'}`;
